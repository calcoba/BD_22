...
def evaluate_test_set(cross_val, df):
    ...
    parameter set.
    The function will plot two measures selected for assessing the performance of the model, the
    coefficient of linear determination (r2) and the Root Mean Squared Error (rmse). For evaluating this models
    the RegressionEvaluator function from pyspark is used.
        :parameter
            cross_val: the Cross Validator model to be fitted and transformed.
            df: dataframe type variable. The dataframe, vectorized, for implementing the model. A column with the features
            and another with the target variable (ArrDelay).
        :returns
